# Engineering Portfolio

Welcome to my Northwind Technologies Enterprise IT Modernization Project.

This portfolio documents the design, implementation, validation, and continuous expansion of the Northwind Technologies Enterprise IT Modernization Project. Through a structured series of implementation sprints, the project demonstrates enterprise infrastructure engineering, systems administration, networking, Help Desk operations, security operations, and professional technical documentation.

## Repository Navigation

| Section | Description |
|---------|-------------|
| Portfolio | Resume, skills, certifications, engineering philosophy |
| Project Management | Charter, roadmap, milestones, risk management |
| Enterprise Documentation | Enterprise architecture and supporting documentation |
| Sprint Documentation | Engineering binders and implementation milestones |
| Diagrams | Network, Active Directory, security, and infrastructure diagrams |
| SOPs | Standard Operating Procedures |
| Incident Reports | Security and operational investigations |
| Ticket Walkthroughs | Help Desk ticket simulations |
| PowerShell | Enterprise automation scripts |

## What This Portfolio Demonstrates

This portfolio demonstrates practical experience in:

- Enterprise infrastructure deployment
- Active Directory administration
- Windows and Linux server administration
- Enterprise networking
- Help Desk engineering
- Technical troubleshooting
- Security validation
- Professional engineering documentation
- Project management
- Change management
- Operational readiness

## Current Project Status

Northwind Technologies
Enterprise IT Modernization Project

Sprint 1
Enterprise Infrastructure
✔ Complete

Sprint 2
Enterprise Operations
In Progress

Sprint 3
Help Desk Operations
Planned

Sprint 4
Security Operations
Planned

Sprint 5
Cloud Administration
Planned

Sprint 6
Automation
Planned

Sprint 7
Enterprise Capstone
Planned

## Engineering Philosophy

The Northwind Technologies Enterprise IT Modernization Project is built upon a set of engineering principles developed throughout each implementation sprint.

These principles guide every deployment, validation, troubleshooting activity, and engineering decision throughout the project.

- Build a stable infrastructure before deploying enterprise services.
- Standardize configurations to simplify administration and troubleshooting.
- Validate every implementation before expanding the environment.
- Troubleshoot methodically using evidence rather than assumptions.
- Document engineering decisions alongside technical procedures.
- Build repeatable, maintainable, and scalable enterprise solutions.

## Enterprise Environment Overview

Northwind Technologies simulates a mid-sized enterprise Information Technology Services department.

Current Environment

• 1 Domain Controller
• 1 Enterprise Workstation
• 1 Ubuntu Enterprise Server
• Active Directory Domain Services
• DNS
• DHCP
• Group Policy
• Enterprise File Services
• Wazuh
• osTicket

Future Expansion

• Microsoft 365
• Azure
• PowerShell Automation
• Cloud Identity
• Enterprise Monitoring

## Featured Engineering Projects

Sprint 1 – Enterprise Infrastructure

Designed and deployed the foundational enterprise infrastructure supporting the Northwind Technologies modernization project.

Key Deliverables

• Oracle VirtualBox Enterprise Environment
• Windows Server 2025 Domain Controller
• Windows 11 Enterprise Workstation
• Ubuntu Enterprise Server
• Active Directory Domain Services
• Enterprise DNS & DHCP
• Engineering Validation Framework
• Enterprise Troubleshooting Guide
• Engineering Documentation Standard

## Upcoming Projects

• Sprint 2 – Enterprise Operations
• Sprint 3 – Help Desk Operations
• Sprint 4 – Security Operations
• Sprint 5 – Cloud Administration
• Sprint 6 – Automation
• Sprint 7 – Enterprise Capstone

## Project Metrics

| Metric | Status |
|---------|--------|
| Sprint Completed | 1 of 7 |
| Engineering Guides | 6+ |
| Standard Operating Procedures | 5 |
| Project Management Documents | 8 |
| Enterprise Documents | 10 |
| Operating Systems Deployed | 3 |
| Virtual Machines | 3 |
| Documentation Standard | Established |
| Repository Status | Active Development |

## Technical Capabilities

Enterprise Infrastructure
    - Infrastructure
    - Identity & Access
    - Networking
    - Operating Systems
    - Documentation
    - Project Management
